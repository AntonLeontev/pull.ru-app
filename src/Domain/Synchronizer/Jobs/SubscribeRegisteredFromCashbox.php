...
class SubscribeRegisteredFromCashbox implements ShouldQueue
{
    ...
    public function handle(UnisenderService $uni)
    {
        try {
            ...
            $message = sprintf(
                'Не удалось подписать пользователя зареганного на кассе на юнисендер: id %s, %s, %s, %s',
                $this->client->id,
                $this->client->name,
                $this->client->phone,
                $this->client->email,
